...
interface LlmResponse {
	...
	responseText: string;
	/** The type of action the UI should take (e.g., 'showLessonOverview', 'showQuiz'). */
...
export default function Chat({
 ...
 const scrollToBottom = () => {
  messagesEndRef.current?.scrollIntoView({ behavior: "smooth" });
	 ...
	 */
	const updateChatMessages = useCallback((type: 'addUser' | 'addAi' | 'addTyping' | 'removeTyping', text?: string) => {
		setMessages(prev => {
			switch (type) {
				case 'addUser':
					...
					break;
				case 'addAi':
					if (text) return [...prev, { text, type: 'ai' }];
					break;
				case 'addTyping':
 ...
 const processSubmission = useCallback(
 	async (messageText: string) => {
 		...
 		updateChatMessages("addAi", responseText);
 	...
 	[
 		isProcessing,
 		chatDb,
 		llmContext, // Keep llmContext as processSubmission reads it directly for history
 		updateChatMessages,
 		callChatApi,
